...
public class InsertPhotoAction extends HttpServlet {
			...
			try {
				...
				while (fii.hasNext()) {
					...
					if (!fis.isFormField() && fis.getName().length() > 0) {
						...
						url = userFilePath+keeno+"/adore/" + photoName;
						...
						AdorePhotoServe adorePhotoServe = (AdorePhotoServe)ctx.getBean("adorePhotoServe");
						adorePhotoServe.addAdorePhoto(keeno, rawPhotoName, photoName, uploadDate+" "+uploadTime);
						
						//生成缩略图
						ImageManager imageManager = new ImageManager();
						imageManager.thumbnail(imageManager, userFilePath+keeno+"/adore/"+photoName, userFilePath+keeno+"/adore_thum/"+photoName, 200, 9000);
