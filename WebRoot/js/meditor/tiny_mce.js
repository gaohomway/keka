...
tinymce.util.Quirks = function(d) {
	...
	function j() {
		function G(J) {
			...
			L = p.getParent(H.startContainer, p.isBlock);
			tinymce.each(p.select(
					"span.Apple-style-span,font.Apple-style-span", L),
					function(M) {
						var N = E.getBookmark();
						if (K) {
		...
		d.onKeyDown.add(function(H, J) {
			var I;
			I = J.keyCode == u;
			if (!J.isDefaultPrevented() && (I || J.keyCode == t)
					&& !m.modifierPressed(J)) {
				J.preventDefault();
				G(I)
			}
		...
		d.parser.addNodeFilter("pre", function(G, I) {
			...
			while (J--) {
				...
				while (H--) {
					...
					K = M.prev;
					if (K && K.type === 3
							&& K.value.charAt(K.value - 1) != "\n") {
						K.value += "\n"
					} else {
						M.parent.insert(new tinymce.html.Node("#text", 3), M,
								true).value = "\n"
					}
				}
			}
	...
	function r() {
		d.onKeyDown.add(function(M, N) {
			var L, G, H, J, K, O, I;
			L = N.keyCode == u;
			if (!N.isDefaultPrevented() && (L || N.keyCode == t)
					&& !m.modifierPressed(N)) {
				G = E.getRng();
				H = G.startContainer;
				J = G.startOffset;
				...
				if (H.nodeType == 3 && H.nodeValue.length > 0
					...
					M.getDoc().execCommand(L ? "ForwardDelete" : "Delete",
							false, null);
					H = E.getRng().startContainer;
					O = H.previousSibling;
					if (O && O.nodeType == 1 && !p.isBlock(O) && p.isEmpty(O)
							&& !nonEmptyElements[O.nodeName.toLowerCase()]) {
						p.remove(O)
					}
					p.remove("__tmp")
				}
	...
	function e() {
		d.onKeyDown.add(function(K, L) {
			...
			while (J && J.parentNode && J.parentNode.firstChild == J
					&& J.parentNode != G) {
				J = J.parentNode
			}
	...
	function g() {
		...
		function Y() {
			function Z(aa, ab) {
				if (ab) {
					if (X.style[aa]
							|| !d.schema.isValid(X.nodeName.toLowerCase(), aa)) {
						p.setStyle(X, aa, ab)
					} else {
						p.setAttrib(X, aa, ab)
					}
					...
					"tinymce.dom.Selection",
					{
						...
						setContent : function(h, j) {
							...
							if (g.insertNode) {
								...
								if (/^\s+/.test(h)) {
									g
											.pasteHTML('<span id="__mce_tmp">_</span>' + h);
					...
					"tinymce.Editor",
					{
						...
						init : function() {
							...
							if (H.document_base_url != k.documentBaseURL) {
								G.iframeHTML += '<base href="' + G.documentBaseURI
										.getURI() + '" />'
							}
							if (H.ie7_compat) {
...
(function(a) {
	a.Formatter = function(aa) {
		var Q = {}, T = a.each, c = aa.dom, r = aa.selection, t = a.dom.TreeWalker, N = new a.dom.RangeUtils(
				c), d = aa.schema.isValidChild, H = c.isBlock, m = aa.settings.forced_root_block, s = c.nodeIndex, G = "\uFEFF", e = /^(src|href|style)$/, X = false, C = true, P, D, x = c.getContentEditable;
		function A(ab) {
